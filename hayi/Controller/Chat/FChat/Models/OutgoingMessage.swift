...
class OutgoingMessage {
    ...
    func sendMessage(chatRoomID: String, messageDictionary: NSMutableDictionary, memberIds: [String], membersToPush: [String]) -> String {
        
        let messageId = UUID().uuidString
        messageDictionary[kMESSAGEID] = messageId
        
        for memberId in memberIds {
            reference(.Message).document(memberId).collection(chatRoomID).document(messageId).setData(messageDictionary as! [String : Any]){(error)in
                               if let err = error {
                                   print(err.localizedDescription)
                                
                                Crashlytics.crashlytics().log("Error: \(err.localizedDescription),Chat room: \(chatRoomID),MemberId :\(memberId), Message Id : \(messageId), Message Dic: \(messageDictionary)")
                                //retry once more
