...
class DImagePostCell: UITableViewCell {
    ...
    @objc func didTap() {
        ...
        self.delegateView.didSelectViewImage(image: image!)
    }
        ...
        if let con = content {
            for x in con {
                if let p = x.path {
                    ...
                    self.kingfisherSource.append(KingfisherSource(urlString: url)!)
                }
            }
        }
    }
...
extension DImagePostCell: ImageSlideshowDelegate {
